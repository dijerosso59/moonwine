		...
		class WC_Chronopost_Product extends WC_Shipping_Method
		{
			...
			public function get_shipping_rate($package = array())
			{
				...
				if ($this->settings['quickcost_enable'] == 'yes') {
					...
					if ($this->id == 'chronoexpress' || $this->id == 'chronoclassic') {
			...
			public function generate_table_rates_table_html()
			{
				...
				<tr>
					...
					<td id="<?php echo $this->id; ?>_settings" class="shipping-rate-table"
						...
						data-countries='<?php echo json_encode($this->country_array); ?>'>
						<table class="shippingrows widefat">
							...
							<tr>
								<td colspan="3" class="add-zone-buttons">
									<a href="#" class="add button"><?php _e('Add New Shipping Zone',
												'chronopost'); ?></a>
									<a href="#" class="delete button"><?php _e('Delete Selected Zones',
												'chronopost'); ?></a>
								</td>
							</tr>
							</tbody>
			...
			public function process_custom_settings()
			{
				...
				if (isset($_POST['woocommerce_' . $this->id . '_enabled']) && $_POST['woocommerce_' . $this->id . '_enabled'] === "1" && !$allowed) {
					if (!defined('CHRONO_RUN_ONCE')) {
						WC_Admin_Settings::add_error(__("You can't enable this product with this contract. Please contact us for more informations.",
								'chronopost'));
...
function chrono_saturday_additional_shipping_field($method)
{
	...
	if (isset($_POST['shipping_method']) && $method->get_id() === $_POST['shipping_method'][0] && chrono_get_saturday_shipping_id($method->get_id())) {
		...
		$enable_frontend_saturday_shipping = (chrono_get_method_settings($method->get_id(),
						'enable_frontend_saturday_shipping') === 'yes');
		$_deliver_on_saturday = (bool)(chrono_get_method_settings($method->get_id(),
...
// Sauvegarde de la données si la livraison le samedi est sélectionnée par le client
